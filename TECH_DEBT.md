...
| `test_secure_user_model_*` (4 tests) | `SecureUser` redefine tabla `users` → conflicto SQLAlchemy con `User` |
...
- **Corregido:** `.github/workflows/ci.yml` con `pnpm run typecheck`, `pnpm run build` y `pytest` (smoke + migraciones).
